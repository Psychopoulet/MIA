...
// private
	...
	function _loadDatabase(Container) {
		...
		return require("node-scenarios").init(path.join(dirDatabase, "MIA.sqlite3")).then((scenariosContainer) => {
			...
			Container	.set("devices", new (require(path.join(dirDatabase, "devices.js")))(Container))
						.set("devicesfunctions", new (require(path.join(dirDatabase, "devicesfunctions.js")))(Container))
						.set("devicestypes", new (require(path.join(dirDatabase, "devicestypes.js")))(Container))
			...
			if (MIADatabaseCreated) {
				...
				return new Promise((resolve, reject) => {
					...
					return Container.get("conditionstypes").add({ code: "HOUR_EQUAL", "name" : "Heure égale à" }).then((conditiontype) => {
						...
						return Container.get("conditions").bindExecuter(conditiontype, (condition) => {
							...
							if (!condition) {
								...
								return Promise.reject("HOUR_EQUAL/executer : Il n'y a pas de paramètres à cette condition");
							}
							else if (!condition.params.hour) {
								return Promise.reject("HOUR_EQUAL/executer : Il n'y a pas d'heure à cette condition");
							}
							else if (!condition.params.minute) {
								return Promise.reject("HOUR_EQUAL/executer : Il n'y a pas de minute à cette condition");
							}
							else {
								return Promise.resolve(d.getHours() == condition.params.hour && d.getMinutes() == condition.params.minute);
							}
						...
						return Container.get("conditions").bindExecuter(conditiontype, (condition) => {
							...
							if (!condition) {
								...
								return Promise.reject("PERIOD_WEEK/executer : Il n'y a pas de paramètres à cette condition");
							}
							else if (!condition.params.days || !(condition.params.days instanceof Array)) {
								return Promise.reject("PERIOD_WEEK/executer : Il n'y a pas de jours liés à cette condition");
							}
							else {
								return Promise.resolve(-1 < condition.params.days.indexOf(d.getDay()));
							}
					...
					// création des données principales des périphériques

					return Container.get("devicesfunctions").add({
						code: "CLIENT",
						name: "client",
						icon: "users"
					}).then((CLIENT) => {

						return Container.get("devicestypes").add({
							function: CLIENT,
							code: "WEB",
							name: "navigateur",
							icon: "desktop"
