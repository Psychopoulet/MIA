...
// private
	...
	function _loadDatabase(Container) {
		...
		return require("node-scenarios").init(path.join(__dirname, "..", "database", "MIA.sqlite3")).then((container) => {
			...
			return new Promise((resolve, reject) => {
				...
				Container.get("db").all("SELECT name, type FROM sqlite_master WHERE type='table' AND name='users';", [], (err, rows) => {
