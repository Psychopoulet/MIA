		...
		searchLinkedActions (cron) {
			...
			if (!cron) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.all("SELECT id_action FROM crons_actions WHERE id_cron = :id;", { ":id" : cron.id }, (err, rows) => {
		...
		searchLinkedScenarios (cron) {
			...
			if (!cron) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.all("SELECT id_scenario FROM crons_scenarios WHERE id_cron = :id;", { ":id" : cron.id }, (err, rows) => {
