...
// private
	...
	// attrs
		...
		" FROM crons" +
			" INNER JOIN triggers ON triggers.id = crons.id_trigger" +
...
module.exports = class DBCrons extends require("node-scenarios").abstract {
	...
	// formate data
		...
		static formate(cron) {
			...
			cron.trigger = {
				id : cron.trigger_id,
				code : cron.trigger_code,
				name : cron.trigger_name
			};

				delete cron.trigger_id;
				delete cron.trigger_code;
				delete cron.trigger_name;
			...
			return new Promise((resolve, reject) => {
				...
				if ("object" === typeof data && null != data) {
					...
					if (data.id) {
						...
						if ("object" === typeof data.trigger && null != data.trigger) {

							if (data.trigger.id) {
								query += " AND triggers.id = :trigger_id";
								options[":trigger_id"] = data.trigger.id;
							}
							else if (data.trigger.code) {
								query += " AND triggers.code = :trigger_code";
								options[":trigger_code"] = data.trigger.login;
							}
							else {

								if (data.trigger.name) {
									query += " AND triggers.name = :trigger_name";
									options[":trigger_name"] = data.trigger.name;
								}
						...
						if ("object" === typeof data.user && null != data.user) {
							...
							if (data.user.id) {
								...
								options[":user_id"] = data.user.id;
							}
							else if (data.user.login) {
								query += " AND users.login = :user_login";
								options[":user_login"] = data.user.login;
							}
							else {

								if (data.user.email) {
									query += " AND users.email = :user_email";
									options[":user_email"] = data.user.email;
								}
	...
	// write
		...
		add (cron) {
			...
			if (!cron) {
				...
				return Promise.reject("Aucun déclencheur renseigné.");
			}
				else if (!cron.trigger.id) {
					return Promise.reject("Le déclencheur renseigné n'est pas valide.");
				}
			else if (!cron.user) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("INSERT INTO crons (id_trigger, id_user, code, name, timer) VALUES (:id_trigger, :id_user, :code, :name, :timer);", {
						":id_trigger": cron.trigger.id,
		...
		edit (cron) {
			...
			if (!cron) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("UPDATE crons SET id_trigger = :id_trigger, name = :name, timer = :timer WHERE id = :id;", {
						":id": cron.id,
						":id_trigger": cron.trigger.id,
						":name": cron.name,
						":timer": cron.timer.string
