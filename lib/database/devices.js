...
module.exports = class DBDevices extends require("node-scenarios").abstract {
	...
	// formate data
		...
		static formate(device) {
			...
			device.id = ("undefined" === typeof device.id) ? null : device.id;
			device.token = ("undefined" === typeof device.token) ? null : device.token;
			device.name = ("undefined" === typeof device.name) ? null : device.name;

			device.function = ("undefined" === typeof device.devices_functions_id) ? null : {
			...
			if ("undefined" === typeof device.status_id && "undefined" === typeof device.status) {
				device.status = null;
			}
			else {

				device.status = (device.status) ? device.status : DBStatus.formate({
					id : device.status_id,
			...
			device.type = ("undefined" === typeof device.devices_types_id) ? null : {
			...
			device.user = ("undefined" === typeof device.user_id) ? null : DBUsers.formate({
				id : device.user_id,
	...
	// write
		...
		add (device) {
			...
			if ("undefined" === typeof device) {
			...
			else if ("undefined" === typeof device.function) {
				return Promise.reject("Aucune fonction renseignée.");
			}
				else if ("undefined" === typeof device.function.id) {
					return Promise.reject("La fonction renseignée n'est pas valide.");
				}
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("INSERT INTO devices (id_function, id_status, id_type, id_user, token, name) VALUES (:id_function, :id_status, :id_type, :id_user, :token, :name);", {
						":id_function": device.function.id,
