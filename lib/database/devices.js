			...
			return new Promise((resolve, reject) => {
				...
				this.db.get(_sSelectQuery + " ORDER BY devices.id DESC LIMIT 0,1;", [], (err, row) => {
					...
					if (err) {
						...
						resolve((row) ? DBDevices.formate(row) : null);
					}
				...
				if ("object" === typeof data && null != data) {
					...
					if (data.id) {
						...
						if ("object" === typeof data.function && null != data.function) {
							...
							if (data.function.id) {
								query += " AND devices_functions.id = :devices_functions_id";
								options[":devices_functions_id"] = data.function.id;
							}
							else if (data.function.code) {
								query += " AND devices_functions.code = :devices_functions_code";
								options[":devices_functions_code"] = data.function.code;
							}
							else {
						...
						if ("object" === data.type && null != data.type) {
							...
							if (data.type.id) {
								...
								query += " AND devices_types.code = :devices_types_code";
								options[":devices_types_code"] = data.type.code;
							}
							else {
			...
			if (!device) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("INSERT INTO devices (id_status, id_type, id_user, token, name) VALUES (:id_status, :id_type, :id_user, :token, :name);", {
						...
						":id_user": ("object" === typeof device.user && device.user.id) ? device.user.id : null,
		...
		edit (device) {
			...
			if (!device) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("UPDATE devices SET id_status = :id_status, id_type = :id_type, id_user = :id_user, name = :name WHERE id = :id;", {
						":id": device.id,
						":id_status": device.status.id,
						":id_type": device.type.id,
