		...
		container.set(
			...
			container.get("servers.app").get("/api/devices", (req, res) => {
				...
				container.get("servers.web").checkAPI_userAllowedOrWaiting(req, res).then((myDevice) => {
					...
					if (!req.params || "string" !== typeof req.params.token) {
						container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devices-searchone-020", message: "Il manque le token" } ]);
					}
						else if ("" === req.params.token.trim()) {
							container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devices-searchone-021", message: "Le token est vide" } ]);
						}
					else {
				...
				container.get("servers.web").checkAPI_userAllowedOrWaiting(req, res).then(() => {
					...
					if (!req.body) {
						...
						container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devices-delete-030", message: "Il manque le périphérique" } ]);
					}
						else if ("string" !== typeof req.params.token) {
							container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devices-delete-031", message: "Le périphérique n'a pas de token" } ]);
						}
						else if ("" === req.params.token.trim()) {
							container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devices-delete-032", message: "Le token du périphérique est vide" } ]);
						}
					else {
						...
						container.get("devices").searchOne({ token: req.params.token.trim() }).then((device) => {
							...
							if (!device) {
								container.get("servers.web").sendNotLoggedJSONResponse(res);
							}
							else {
