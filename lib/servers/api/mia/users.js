...
module.exports = (container) => {
	...
	container.set(
		...
		container.get("servers.app").get("/api/users", (req, res) => {
			...
			if (!req.body) {
				container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "users-login-010", message: "Il n'y a aucune donnée" } ]);
			}
			else if ("string" !== typeof req.body.token) {
				container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "users-login-020", message: "Il manque le token" } ]);
			}
				else if ("" === req.body.token.trim()) {
					container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "users-login-021", message: "Le token est vide" } ]);
				}
				...
				container.get("devices").searchOne({ token: req.body.token.trim() }).then((device) => {
					...
					if (!device) {
						container.get("servers.web").sendNotLoggedJSONResponse(res, [ { code: "users-login-22", message: "Le périphérique lié à ce token n'existe pas" } ]);
					}
					else {
						...
						container.get("users").searchOne({
							...
							if (!user) {
								container.get("servers.web").sendNotLoggedJSONResponse(res, [ { code: "users-login-040", message: "Cet utilisateur n'existe pas" } ]);
							}
							else {
								...
								container.get("status").searchOne({ code: "ACCEPTED" }).then((status) => {
									...
									if (!status) {
										container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "users-login-050", message: "Ce statut n'existe pas" } ]);
									}
									else {
