		...
		container.get("servers.app").get("/api/status", (req, res) => {
			...
			container.get("servers.web").checkAPI_userAllowed(req, res).then(() => {
				...
				if (!req.params || "string" !== typeof req.params.code) {
					container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "status-searchone-020", message: "Il manque le code" } ]);
				}
					else if ("" === req.params.code.trim()) {
						container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "status-searchone-021", message: "Le code est vide" } ]);
					}
				else {
