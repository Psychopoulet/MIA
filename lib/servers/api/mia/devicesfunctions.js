		...
		container.set(
			...
			container.get("servers.app").get("/api/devicesfunctions", (req, res) => {
				...
				container.get("devicesfunctions").search().then((devicesfunctions) => {
					container.get("servers.web").sendValidJSONResponse(req, res, devicesfunctions);
				}).catch((err) => {
				...
				if (!req.params || "string" !== typeof req.params.code) {
					container.get("servers.web").sendWrongRequestJSONResponse(req, res, [ { code: "devicesfunctions-searchone-020", message: "Il manque le code" } ]);
				}
					else if ("" === req.params.code.trim()) {
						container.get("servers.web").sendWrongRequestJSONResponse(req, res, [ { code: "devicesfunctions-searchone-021", message: "Le code est vide" } ]);
					}
				else {

					container.get("devicesfunctions").searchOne({ code: req.params.code.trim().toUpperCase() }).then((devicefunction) => {
						container.get("servers.web").sendValidJSONResponse(req, res, devicefunction);
					}).catch((err) => {
