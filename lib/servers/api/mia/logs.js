	...
	container.set(
		...
		container.get("servers.app").get("/api/logs", (req, res) => {
			...
			container.get("logs").getLogs().then(function(logs) {
				...
				container.get("servers.web").sendValidJSONResponse(req, res, logs);
			...
			container.get("logs").getLogs().then(function(logs) {
				...
				if (logs[req.params.year]) {
					container.get("servers.web").sendValidJSONResponse(req, res, logs[req.params.year]);
				}
				else {
					container.get("servers.web").sendValidJSONResponse(req, res, null);
				}
			...
			container.get("logs").getLogs().then(function(logs) {
				...
				if (logs[req.params.year] && logs[req.params.year][req.params.month]) {
					container.get("servers.web").sendValidJSONResponse(req, res, logs[req.params.year][req.params.month]);
				}
				else {
					container.get("servers.web").sendValidJSONResponse(req, res, null);
				}
			...
			container.get("logs").getLogs().then(function(logs) {
				...
				if (logs[req.params.year] && logs[req.params.year][req.params.month] && logs[req.params.year][req.params.month][req.params.day]) {
					container.get("servers.web").sendValidJSONResponse(req, res, logs[req.params.year][req.params.month][req.params.day]);
				}
				else {
					container.get("servers.web").sendValidJSONResponse(req, res, null);
				}
			...
			container.get("logs").read(req.params.year, req.params.month, req.params.day, req.params.number).then(function(HTMLLogs) {
				...
				container.get("servers.web").sendValidHTMLResponse(req, res, HTMLLogs);

			}).catch(function(err) {

				err = (err.message) ? err.message : err;

				container.get("servers.web").sendInternalErrorHTMLResponse(req, res, err);
