		...
		container.set(
			...
			container.get("servers.app").get("/api/devicestypes", (req, res) => {
				...
				container.get("devicestypes").search().then((devicestypes) => {
					container.get("servers.web").sendValidJSONResponse(res, devicestypes);
				}).catch((err) => {
				...
				if (!req.params || "string" !== typeof req.params.code) {
					container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devicestypes-searchone-020", message: "Il manque le code" } ]);
				}
					else if ("" === req.params.code.trim()) {
						container.get("servers.web").sendWrongRequestJSONResponse(res, [ { code: "devicestypes-searchone-021", message: "Le code est vide" } ]);
					}
				else {

					container.get("devicestypes").searchOne({ code: req.params.code.trim().toUpperCase() }).then((deviceType) => {
						container.get("servers.web").sendValidJSONResponse(res, deviceType);
					}).catch((err) => {
