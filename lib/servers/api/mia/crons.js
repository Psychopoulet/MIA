				...
				container.get("servers.web").checkAPI_userAllowed(req, res).then(() => {
					...
					if ("object" !== typeof req.params) {
						...
						container.get("crons").searchOne({ code: croncode }).then((cron) => {
							...
							container.get("crons").searchLinkedActions(cron).then((actionsIds) => {

								container.get("servers.web").sendWrongRequestJSONResponse(req, res, [ { code: "crons-actions-030", message: "En cours de construction", data: actionsIds } ]);
								...
								container.get("servers.web").sendInternalErrorJSONResponse(req, res, err);
								container.get("logs").err("-- [database/crons/searchlinkedactions] " + err);
				...
				container.get("servers.web").checkAPI_userAllowed(req, res).then(() => {
					...
					if ("object" !== typeof req.params) {
						...
						container.get("crons").searchOne({ code: croncode }).then((cron) => {
							...
							container.get("crons").searchLinkedScenarios(cron).then((scenariosIds) => {

								container.get("servers.web").sendWrongRequestJSONResponse(req, res, [ { code: "crons-scenarios-030", message: "En cours de construction", data: scenariosIds } ]);
