...
module.exports = class ServerWeb {
	...
	generateBuffers() {
		...
		let indexBufferFile = path.join(m_sDirBuffers, "index.html"),
			...
			designsBufferFile = path.join(m_sDirBuffers, "css", "designs.css"),
		...
		.then(() => {
			...
			return fs.mkdirpProm(path.join(m_sDirBuffers, "css"));
		}).then(() => {
		...
		.then(() => {
			...
			return _bufferPlugin(0).then(() => {
				return fs.readFileProm(path.join(m_sDirWeb, "templates", "index.html"), "utf8");
			}).then((index) => {
