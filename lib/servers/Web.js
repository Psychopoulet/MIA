	...
	start () {
		...
		return this.multiserver.addServer({
			...
			let express = require("express"),
				...
				.use("/libs/fonts/", express.static(path.join(_dirWeb, "libs", "font-awesome-4.5.0")));
				...
				app.use((req, res) => {
					this.sendHTMLResponse(res, 404, "404 : url not found");
				});
	...
	// API
		...
		sendResponse(res, code, contentType, msg) {
			...
			sendHTMLResponse(res, code, msg) {
				...
				sendValidHTMLResponse(res, msg) {
					this.sendHTMLResponse(res, 200, msg);
				}

				// errors

				sendInternalErrorHTMLResponse(res, msg) {
					this.sendHTMLResponse(res, 500, msg);
				}

			// JSON

			sendJSONResponse(res, code, msg) {
				this.sendResponse(res, code, "application/json; charset=utf-8", JSON.stringify(msg));
			}
				...
				sendErrorJSONResponse(res, code, msg) {
					...
					this.sendJSONResponse(res, code, { status: code, errors: msg });
					
				}

					sendWrongRequestJSONResponse(res, msg) {
						this.sendErrorJSONResponse(res, 400, msg);
					}

					sendNotLoggedJSONResponse(res, msg) {
						this.sendErrorJSONResponse(res, 401, [ { code: "notlogged", message: msg } ]);
					}

					sendNotAllowedJSONResponse(res) {
						this.sendErrorJSONResponse(res, 403, [ { code: "notallowed", message: "Vous n'êtes pas autorisé à acceder à cette fonctionnalité" } ]);
					}

					sendNotAcceptableJSONResponse(res, msg) {
						this.sendErrorJSONResponse(res, 406, [ { code: "notacceptable", message: msg } ]);
					}

					sendInternalErrorJSONResponse(res, msg) {
						this.sendErrorJSONResponse(res, 500, [ { code: "internalerror", message: msg } ]);
					}
		...
		checkAPI_accept(req, res) {
			...
			if (!req.accepts("application/json")) {
				this.sendNotAcceptableJSONResponse(res, "Seul le type de requête 'application/json' est pris en charge par les API du serveur");
				...
				this.sendNotAcceptableJSONResponse(res, "Seul le type de charset 'utf-8' est pris en charge par les API du serveur");
			...
			checkAPI_token(req, res) {
				...
				return this.checkAPI_accept(req, res).then(() => {
					...
					if (!req.headers) {
						this.sendWrongRequestJSONResponse(res, [ { code: "authorization-01", message: "Il n'y a aucune donnée d'en-tête" } ]);
						return Promise.reject("Il n'y a aucune donnée d'en-tête");
					}
					else if ("string" !== typeof req.headers.authorization) {
						this.sendWrongRequestJSONResponse(res, [ { code: "authorization-02", message: "Il manque le token d'autorisation dans les données d'en-tête" } ]);
						return Promise.reject("Il manque le token d'autorisation dans les données d'en-tête");
					}
					else if ("" === req.headers.authorization.trim()) {
						this.sendWrongRequestJSONResponse(res, [ { code: "authorization-03", message: "Le token d'autorisation des données d'en-tête est vide" } ]);
					...
					return this.checkAPI_token(req, res).then((token) => {
						...
						return new Promise((resolve, reject) => {
							...
							this.container.get("devices").searchOne({ token: token }).then((device) => {
								...
								if (!device) {
									this.sendNotLoggedJSONResponse(res, "Le périphérique n'existe pas");
									reject("Le périphérique n'existe pas");
								}
								else if (!device.status || "string" !== typeof device.status.code) {
									this.sendNotLoggedJSONResponse(res, "Le périphérique n'a pas de statut");
									reject("Le périphérique n'a pas de statut");
								}
								else if ("BLOCKED" === device.status.code) {
									this.sendNotLoggedJSONResponse(res, "Le périphérique est bloqué");
									reject("Le périphérique est bloqué");
								}
								else if ("WAITING" === device.status.code) {
									this.sendNotLoggedJSONResponse(res, "Le périphérique n'a pas encore été accepté");
									reject("Le périphérique n'a pas encore été accepté");
								}
								else if ("ACCEPTED" !== device.status.code) {
									this.sendNotLoggedJSONResponse(res, "Le périphérique a un statut inconnu ('" + device.status.code + "')");
								...
								this.container.get("servers.web").sendInternalErrorJSONResponse(res, err);
		...
		.then(() => {
			...
			function _bufferPlugin(i) {
				...
				if (i >= plugins.length) {
					...
					return fs.concatFilesProm(plugins[i].templates, "utf8", "\r\n").then((templates) => {
						...
						return fs.concatFilesProm(plugins[i].designs, "utf8", "\r\n").then((designs) => {
			...
			return fs.concatDirectoryFilesProm(path.join(_dirWeb, "js")).then((data) => {
			...
			// on recopie les libs
				...
				.then(() => {
					...
					return fs.concatDirectoryFilesProm(path.join(dirLibs, "angularjs", "modules")).then((data) => {
