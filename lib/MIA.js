...
// private
	...
	// methods
		...
		function _runCron (that, cron) {
			...
			try {
				...
				cron.job = new cronjob(cron.timer, function() {
					...
					that.container.get("actions").getAllByCron(cron).then(function(actions) {
		...
		function _classicActionExecuter (that, action) {
			...
			if (action.child && action.child.token) {
				...
				if ("object" === typeof action.params) {
					that.container.get("servers.children.sockets").emitTo(action.child.token, action.type.command, action.params);
				}
				else if ("string" === typeof action.params) {
					that.container.get("servers.children.sockets").emitTo(action.child.token, action.type.command, JSON.parse(action.params));
				}
				else {
					that.container.get("servers.children.sockets").emitTo(action.child.token, action.type.command);
				}
				...
				if ("object" === typeof action.params) {
					that.container.get("servers.children.sockets").emit(action.type.command, action.params);
				}
				else if ("string" === typeof action.params) {
					that.container.get("servers.children.sockets").emit(action.type.command, JSON.parse(action.params));
				}
				else {
					that.container.get("servers.children.sockets").emit(action.type.command);
				}
		...
		function _startCrons(that) {
			...
			return that.container.get("crons").getAll().then(function(crons) {
		...
		function _startExecuters(that) {
			...
			return that.container.get("actionstypes").searchOne({ code: "PLAYSOUNDONCHILD" }).then(function(actiontypeplaysound) {

				return that.container.get("actions").bindExecuter(actiontypeplaysound, function(action) {
			...
			}).then(function(actiontypetts) {

				return that.container.get("actions").bindExecuter(actiontypetts, function(action) {
					_classicActionExecuter(that, action);
				});
