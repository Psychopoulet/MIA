		...
		// crons
			...
			function _runCron (that, cron) {
				...
				try {
					...
					cron.job = new cronjob(that.container.get("crons").cronToTimerString(cron), () => {

						that.container.get("actions").search({ cron: cron }).then((actions) => {
			...
			function _startCrons(that) {
				...
				return that.container.get("crons").search().then((crons) => {
