...
module.exports = class ServerWeb {
	...
	start () {
		...
		return this.multiserver.addServer({
			...
			let express = require("express"),
				...
				.use("/libs/bootstrap.min.css.map", express.static(path.join(_dirWeb, "libs", "bootstrap", "css", "bootstrap.min.css.map")));
				...
				checkAPI_userAllowed(req, res) {
					...
					return this.checkAPI_token(req, res).then((token) => {
						...
						return new Promise((resolve, reject) => {
							...
							this.container.get("controllers").searchOne({ token: token }).then((controller) => {

								if (!controller) {
									this.sendNotLoggedJSONResponse(req, res, "Le contrôleur n'existe pas");
									reject("Le contrôleur n'existe pas");
								}
								else if (!controller.status || "string" !== typeof controller.status.code) {
									this.sendNotLoggedJSONResponse(req, res, "Le contrôleur n'a pas de statut");
									reject("Le contrôleur n'a pas de statut");
								}
								else if ("BLOCKED" === controller.status.code) {
									this.sendNotLoggedJSONResponse(req, res, "Le contrôleur est bloqué");
									reject("Le contrôleur est bloqué");
								}
								else if ("WAITING" === controller.status.code) {
									this.sendNotLoggedJSONResponse(req, res, "Le contrôleur n'a pas encore été accepté");
									reject("Le contrôleur n'a pas encore été accepté");
								}
								else if ("ACCEPTED" !== controller.status.code) {
									this.sendNotLoggedJSONResponse(req, res, "Le contrôleur a un statut inconnu ('" + controller.status.code + "')");
									reject("Le contrôleur a un statut inconnu ('" + controller.status.code + "')");
								}
								else {
									resolve(controller);
								}
								...
								this.container.get("logs").err("-- [database/controllers/searchOne] " + err);
					...
					return this.checkAPI_token(req, res).then((token) => {
						...
						return new Promise((resolve, reject) => {
							...
							this.container.get("controllers").searchOne({ token: token }).then((controller) => {
								...
								else if ("ACCEPTED" !== controller.status.code && "WAITING" !== controller.status.code) {
									this.sendNotLoggedJSONResponse(req, res, "Le contrôleur a un statut inconnu ('" + controller.status.code + "')");
									reject("Le contrôleur a un statut inconnu ('" + controller.status.code + "')");
								}
								else {
	...
	generateBuffers() {
		...
		.then(() => {
			...
			return fs.extractFilesProm(path.join(_dirWeb, "js", "mia")).then((files) => {

				jsfiles = jsfiles.concat(files);
				return fs.extractFilesProm(path.join(_dirWeb, "js", "scenarios"));

			}).then((files) => {

				jsfiles = jsfiles.concat(files);
				return fs.filesToFileProm(jsfiles, javascriptsBufferFile, "\r\n\r\n// ---- {{filename}} ----\r\n\r\n");
			...
			// on recopie les libs
				...
				return fs.copyFileProm(path.join(dirLibs, "bootstrap", "css", "bootstrap.min.css"), path.join(dirLibPublic, "bootstrap.css"))

				// js

				.then(() => {
					return fs.copyFileProm(path.join(dirLibs, "interactjs", "interact.min.js"), path.join(dirLibPublic, "interact.js"));
				}).then(() => {
					...
					return fs.copyFileProm(path.join(dirLibs, "socketio", "socket.io.js"), path.join(dirLibPublic, "socketio.js"));
				}).then(() => {
					return fs.copyFileProm(path.join(dirLibs, "angularjs", "angular.min.js"), path.join(dirLibPublic, "angular.js"));
				}).then(() => {
					return fs.directoryToFileProm(path.join(dirLibs, "angularjs", "modules"), path.join(dirLibPublic, "angular-modules.js"), "\r\n\r\n// ---- {{filename}} ----\r\n\r\n");
				});
